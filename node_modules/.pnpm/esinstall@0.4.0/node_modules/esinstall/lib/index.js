...
function resolveWebDependency(dep, { cwd, packageLookupFields }) {
    ...
    if (packageEntrypoint) {
        ...
        if (packageManifestLoc && packageManifest && packageManifest.exports) {
            ...
            return {
                type: util_2.isJavaScript(loc) ? 'JS' : 'ASSET',
                loc,
            };
        }
    }
    ...
    if (!depManifest) {
        try {
            ...
            return {
                type: util_2.isJavaScript(maybeLoc) ? 'JS' : 'ASSET',
                loc: maybeLoc,
            };
        }
        catch (err) {
...
async function install(_installTargets, _options = {}) {
    var _a;
    const { cwd, alias: installAlias, lockfile, logger, dest: destLoc, namedExports, externalPackage, externalPackageEsm, sourceMap, env: userEnv, rollup: userDefinedRollup, treeshake: isTreeshake, polyfillNode, packageLookupFields, } = setOptionDefaults(_options);
